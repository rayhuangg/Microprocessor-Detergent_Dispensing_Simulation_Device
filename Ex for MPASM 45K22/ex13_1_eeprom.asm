...
;
			...
			movlw	b'00001100'	; Do nothing if TxD_Flag(2:3)=00
			andwf	TxD_Flag, w
			btfsc	STATUS, Z
			bra		UART_DIS
;
...
;
			movlw	EEP_ADRL
			movwf	Data_EE_Addr
			movff	ADRL, Data_EE_Data
			call	WRITE_EEPROM
			movff	EEDATA, ADRL
			movlw	EEP_ADRH
			movwf	Data_EE_Addr
			movff	ADRH, Data_EE_Data
			call	WRITE_EEPROM
			movff	EEDATA, ADRH
			bcf		TxD_Flag, 3
;
...
;
UART_DIS:
 			movf	ADRH,W		; 將轉換結果A/D <b9:b8>轉成ASCII 並由UART送出
 			andlw	b'00000011'
 			call	Hex_ASCII
 			call 	Tx_a_Byte
;
 			swapf	ADRL,W		; 將轉換結果A/D <b7:b4>轉成ASCII 並由USART送出
 			andlw	h'0F'
 			call	Hex_ASCII
 			call	Tx_a_Byte
;
 			movf	ADRL,W		; 將轉換結果A/D <b3:b0>轉成ASCII 並由USART送出
 			andlw	h'0F'
 			call	Hex_ASCII
 			call	Tx_a_Byte
;
...
;
			bcf		PIR1,RCIF				; 清除資料接收中斷旗標
			movff	RCREG, RX_Temp			; 將接收資料顯示在LED
...
No_EQU_c	movlw	a'p'					; 檢查接收資料是否為’p’?
			cpfseq  RX_Temp
			bra		No_EQU_p				; No, 不作動
			bsf		TxD_Flag,1				; Yes, 停止資料傳輸
			bra		Exit_Low_ISR
No_EQU_p	movlw	a'r'					; 檢查接收資料是否為'r' ?
			cpfseq  RX_Temp
			bra		No_EQU_r			; No, 不作動
			bcf		TxD_Flag,3				; 設定讀取EEPROM旗標
			bsf		TxD_Flag,2				; 
			bcf		TxD_Flag,1				; Yes, 啟動傳輸資料
			bra		Exit_Low_ISR
No_EQU_r	movlw	a'w'					; 檢查接收資料是否為'w'?
			cpfseq  RX_Temp
			bra		No_EQU_w			; No, 不作動
			bsf		TxD_Flag,3				; 設定寫入EEPROM旗標
			bcf		TxD_Flag,2				; 
			bcf		TxD_Flag,1				; Yes, 啟動傳輸資料
			bra		Exit_Low_ISR
No_EQU_w	movlw	a'e'					; 檢查接收資料是否為'e'?
			cpfseq  RX_Temp
			bra		Exit_Low_ISR			; No, 不作動
			bsf		TxD_Flag,2				; 設定清除EEPROM旗標
			bsf		TxD_Flag,3				; 
			bcf		TxD_Flag,1				; Yes, 啟動傳輸資料
			bra		Exit_Low_ISR
